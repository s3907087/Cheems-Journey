...
class GameScene: SKScene {
    ...
    var cameraMovePerSec: CGFloat = 450.0
...
extension GameScene{
    ...
    func createPlayer(){
        player = SKSpriteNode(imageNamed: "cheems")
        player.name = "Player"
    ...
    func moveCamera(){
        let amountToMove = CGPoint(x: cameraMovePerSec * CGFloat(dt), y: 0.0)
    ...
    func movePlayer() {
        let amountToMove = cameraMovePerSec * CGFloat(dt)
    ...
    func setUpCoin() {
        ...
        coin.position = CGPoint(x: cameraRect.maxX + coin.frame.height, y: size.height/2.0 + random)
